...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = posts.find((p) => p.title.toLowerCase().replace(/ /g, "-") === resolvedParams.slug);
  ...
  return {
    title: `${post.title} | BizzGrowLabs Blog`,
...
const renderBlock = (block: ContentBlock, index: number) => {
  switch (block.type) {
    case "paragraph":
      return (
        <p key={index} className="text-lg md:text-xl leading-[1.8] text-foreground/80 mb-8 font-medium">
          {block.text}
        </p>
      );
    case "heading":
      const sizeClass = block.level === 2 ? "text-3xl md:text-4xl" : "text-2xl md:text-3xl";
    ...
    case "highlight":
      return (
        <div key={index} className="my-16 md:my-24 py-8 md:py-12 border-y border-border relative">
    ...
    case "quote":
      return (
        <blockquote key={index} className="my-12 p-8 md:p-12 bg-surface border border-border rounded-3xl relative">
          <span className="absolute top-4 left-6 text-6xl text-brand-primary/20 font-serif">"</span>
      ...
      return (
        <figure key={index} className="my-12 w-full">
          <div className="w-full h-[40vh] md:h-[60vh] bg-surface rounded-3xl overflow-hidden relative border border-border">
            ...
            <div className="absolute inset-0 bg-gradient-to-tr from-brand-primary/20 to-brand-mint/20" />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={block.url} alt={block.alt} className="w-full h-full object-cover mix-blend-luminosity opacity-50" />
          </div>
          {block.caption && (
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const toc = post.content
    .filter((block): block is { type: "heading"; text: string; level: 2 | 3 } => block.type === "heading" && block.level === 2)
    .map((h) => ({ title: h.text, id: h.text.toLowerCase().replace(/ /g, "-") }));
          ...
          <header className="container mx-auto px-4 md:px-6 lg:px-8 max-w-7xl mb-12 md:mb-20">
            ...
            <div className="flex items-center gap-4 py-6 border-y border-border">
              ...
              <div>
                ...
                <div className="text-xs font-mono uppercase tracking-widest text-muted">{post.author.role}</div>
              </div>
            </div>
          </header>

          {/* Dynamic Color Banner */}
          <div className={`w-full h-[40vh] md:h-[60vh] ${post.color} relative overflow-hidden mb-16 md:mb-32 flex items-center justify-center banner-image`}>
             <div className="absolute inset-0 opacity-[0.05] mix-blend-overlay pointer-events-none" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")` }} />
             <h2 className={`text-[15vw] font-black tracking-tighter uppercase opacity-[0.03] ${textColor} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap`}>
                {post.category}
             </h2>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_2.5fr] gap-12 lg:gap-24 relative">
              ...
              <aside className="hidden lg:block relative">
                <div className="sticky top-40 flex flex-col gap-12 toc-sidebar">
                  {toc.length > 0 && (
                    <div>
                      <h4 className="text-xs font-mono uppercase tracking-widest text-muted mb-6">On this page</h4>
                      <ul className="flex flex-col gap-4">
                        {toc.map((item, i) => (
                          <li key={i}>
                            <a href={`#${item.id}`} className="text-sm font-medium text-foreground hover:text-brand-primary transition-colors hover:translate-x-1 inline-block duration-300">
                              {item.title}
                            </a>
                          </li>
                        ))}
                  ...
                  <div>
                    <h4 className="text-xs font-mono uppercase tracking-widest text-muted mb-6">Share</h4>
              ...
              <div className="max-w-3xl blog-content">
                ...
                <div className="mt-24 pt-12 border-t border-border flex flex-col md:flex-row items-center justify-between gap-8">
                   <div className="flex items-center gap-4">
                     {/* eslint-disable-next-line @next/next/no-img-element */}
                     <img src={post.author.avatar} alt={post.author.name} className="w-16 h-16 rounded-full border border-border" />
                     <div>
                       <div className="text-xs font-mono uppercase tracking-widest text-muted mb-1">Written by</div>
                       <div className="font-bold text-foreground text-xl">{post.author.name}</div>
                     </div>
                   </div>
