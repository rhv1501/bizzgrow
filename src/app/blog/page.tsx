...
export default function BlogPage() {
  ...
  const blogJsonLd = toJsonLd(
    buildCollectionPageSchema(
      "BizzGrowLabs Blog",
      "Practical articles on websites, search visibility, performance, and growing online.",
      `${baseUrl}/blog`,
    ),
  );
  const blogListJsonLd = toJsonLd(
    buildItemListSchema(
      "BizzGrowLabs Blog",
      posts.map((post) => ({
        name: post.title,
  ...
  return (
    <main className="bg-background min-h-screen selection:bg-brand-mint selection:text-foreground">
      ...
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: blogListJsonLd }}
      />
      ...
      >
          ...
          <h1 className="text-[clamp(4rem,12vw,15rem)] font-medium tracking-tighter leading-[0.8] text-foreground flex flex-col items-center justify-center">
            <span className="flex items-center">
              ...
              <motion.span
                ...
                className="text-brand-primary mx-4 font-serif italic text-[clamp(5rem,14vw,18rem)] leading-none inline-block origin-center mt-4"
              >
                *
              </motion.span>
            </span>
            <span className="italic font-serif opacity-80 pl-[10vw]">
              ALOUD.
            </span>
          </h1>
        </motion.div>
        ...
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 z-10">
          ...
          <motion.div
            ...
            className="w-px h-16 bg-linear-to-b from-foreground to-transparent"
          />
        </div>
      </section>
      ...
      <section
        ref={sectionRef}
        className="relative h-screen w-full overflow-hidden bg-surface flex items-center border-y border-border/60"
      >
        {/* A giant background text that stays fixed while cards scroll over it */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-5">
          <h2 className="text-[30vw] font-black tracking-tighter whitespace-nowrap">
            ARTICLES
          </h2>
        </div>
          ...
          {posts.map((post, i) => {
            ...
            return (
              <Link
                href={`/blog/${post.slug}`}
                key={i}
                className={`group relative shrink-0 w-[85vw] md:w-[45vw] lg:w-[35vw] h-[60vh] md:h-[70vh] rounded-4xl md:rounded-[3rem] p-8 md:p-12 flex flex-col justify-between overflow-hidden shadow-2xl transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-4 ${post.color} ${isEven ? "mt-12" : "-mt-12"}`}
              >
                {/* Texture overlay */}
                <div
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-12">
                    <span
                      className={`px-4 py-2 rounded-full text-xs font-mono uppercase tracking-widest border ${isDark ? "border-background/20 bg-background/10" : "border-foreground/20 bg-foreground/5"} backdrop-blur-md ${textClass}`}
                    >
                      {post.category}
                    </span>
                    <span
                      className={`font-mono text-sm font-bold ${textClass}`}
                    >
                      0{i + 1}
                    </span>
                  </div>
                  ...
                  <h3
                    className={`max-w-[12ch] text-[clamp(1.75rem,2.6vw,3rem)] md:text-[clamp(2rem,2.8vw,3.5rem)] font-medium tracking-tight leading-tight text-balance line-clamp-3 overflow-hidden ${textClass}`}
                  >
                    {post.title}
                  </h3>
                </div>
                ...
                <div className="relative z-10 flex min-h-0 flex-1 flex-col">
                  <div
                    className={`mt-auto flex flex-wrap items-center justify-between gap-3 border-t pt-5 md:pt-6 ${isDark ? "border-background/20" : "border-foreground/20"}`}
                  >
                    <div
                      className={`flex flex-wrap items-center gap-3 md:gap-4 text-[10px] md:text-xs font-mono uppercase tracking-widest ${mutedClass}`}
                    >
                      <span>{post.date}</span>
                      <span
                        className={`w-1 h-1 rounded-full ${isDark ? "bg-background/40" : "bg-foreground/40"}`}
                      />
                      <span>{post.readTime}</span>
                    </div>
                    <div
                      className={`w-10 h-10 md:w-12 md:h-12 rounded-full border flex items-center justify-center transform group-hover:scale-110 transition-all duration-300 ${isDark ? "border-background/20 text-background group-hover:bg-background group-hover:text-foreground" : "border-foreground/20 text-foreground group-hover:bg-foreground group-hover:text-background"}`}
                    >
