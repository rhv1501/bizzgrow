...
export default function PortfolioPage() {
  ...
  const portfolioJsonLd = toJsonLd(
    buildCollectionPageSchema(
      "BizzGrowLabs Portfolio",
      "See BizzGrowLabs case studies, project outcomes, and proof of work across web, SEO, branding, and growth marketing.",
      `${baseUrl}/portfolio`,
    ),
  );
  const portfolioListJsonLd = toJsonLd(
    buildItemListSchema(
      "BizzGrowLabs Portfolio",
  ...
  return (
    ...
    >
      ...
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: portfolioListJsonLd }}
      />

      {/* CINEMATIC HERO */}
      <section className="pt-32 md:pt-40 pb-20 md:pb-24 overflow-hidden relative border-b border-border/60">
        <div className="absolute inset-0 z-0">
          <div className="absolute top-1/4 left-1/4 w-150 h-150 bg-brand-primary/10 rounded-full blur-[120px] mix-blend-multiply pointer-events-none" />
          <div className="absolute bottom-0 right-1/4 w-125 h-125 bg-brand-peach/10 rounded-full blur-[100px] mix-blend-multiply pointer-events-none" />
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div style={{ y: heroY, opacity: heroOpacity }}>
            ...
            <h1 className="text-[clamp(3.5rem,8vw,9rem)] font-medium tracking-tighter text-foreground leading-[0.9]">
              Work that makes <br />
              competitors{" "}
              <span className="italic text-brand-peach font-serif pr-4">
                nervous
              </span>
              .
            </h1>
            <p className="mt-8 md:mt-12 text-xl md:text-2xl text-muted max-w-2xl leading-relaxed">
              Real transformations, measurable results, and digital ecosystems
              that actually look like they were built in this decade.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-32 md:py-48 border-t border-border/60 relative overflow-hidden bg-surface/30">
        <div className="absolute inset-0 z-0 flex items-center justify-center">
          <div className="w-200 h-200 bg-brand-mint/10 blur-[120px] rounded-full pointer-events-none" />
        </div>
        <div className="mx-auto max-w-5xl px-4 relative z-10 text-center">
          <h2 className="text-[clamp(3rem,6vw,6rem)] font-medium tracking-tight mb-12 leading-[1.1]">
            Ready to{" "}
            <span className="italic text-brand-primary font-serif">scale</span>?
          </h2>
    ...
    >
      <div className={`lg:col-span-7 ${isEven ? "lg:order-1" : "lg:order-2"}`}>
        <Link
          ...
          className="block relative rounded-4xl md:rounded-[3rem] overflow-hidden aspect-video bg-surface border border-border/60 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] transform transition-transform duration-800 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.02]"
        >
          {/* Using the gradient or image as the massive background */}
          <div
            className={`absolute inset-0 opacity-80 transition-opacity duration-700 group-hover:opacity-100 ${project.image.startsWith("linear-gradient") ? "mix-blend-multiply" : ""}`}
            style={
      ...
      >
        <span className="text-xs font-mono text-muted uppercase tracking-widest mb-6 block">
          0{index + 1} &mdash; Case Study
        </span>
        ...
        <div className="flex flex-wrap gap-x-12 gap-y-8 w-full">
          {project.results
            .slice(0, 2)
            .map((result: { value: string; label: string }, i: number) => (
              <div key={i} className="flex flex-col flex-1 min-w-35">
                <div className="text-2xl lg:text-3xl font-medium tracking-tight text-foreground mb-2 wrap-break-word leading-tight">
                  {result.value}
                </div>
        ...
        <div className="mt-12 pt-12 border-t border-border/60 flex flex-wrap gap-3">
          {project.technologies.slice(0, 3).map((tech: string, i: number) => (
            <span
              key={i}
              className="text-xs font-mono border border-border/50 rounded-full px-4 py-2 text-muted"
            >
              {tech}
            </span>
          ))}
